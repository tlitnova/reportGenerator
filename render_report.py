...
def build_kpis(data, watchlist):
    ...
    if data.get("sophos_endpoint"):
        ...
        if total:
            pct = round(se.get("active_count", 0) / total * 100)
            candidates.append({"value": f"{pct}%", "label": "Endpoints protected (Sophos)"})
...
def build_recommended_next(watchlist):
    seen, items = set(), []
    for w in watchlist:
    ...
    return items[:8]  # cap so the report stays readable even with a long real watchlist
